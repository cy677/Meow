...
function renderRewards(){
  ...
  for(const [value,name]of [['all','全部'],...Object.entries(labels)]){const b=button(name,'category','chip');b.dataset.category=value;b.setAttribute('aria-pressed',value===category);$('#categories').append(b);}
  const root=$('#reward-grid');root.replaceChildren();
  const items=state.rewards.filter(r=>(category==='all'||r.category===category)&&(view!=='owned'||r.owned));
  if(!items.length){root.append(el('p','empty','这里还没有收藏。去发现一份喜欢的奖励吧。'));return;}
  for(const reward of items){
    ...
    const tag=el('span','reward-tag',reward.equipped?'使用中':reward.owned?'已收藏':reward.eligible?'可以兑换':'成长解锁');art.append(tag);
    const content=el('div','reward-content');content.append(el('h3','',reward.title),el('p','',reward.description));
    const foot=el('div','reward-foot');
    const price=reward.owned?'永久拥有':reward.cost===0?'成长礼物':`${reward.cost} 积分`;foot.append(el('strong','',price));
    let b;
    if(reward.owned){b=button(reward.category==='trick'?'玩一下':reward.equipped?'正在使用':'换上它',reward.category==='trick'?'play':'equip','button small');b.disabled=reward.equipped||!online;}
...
async function updateScene(){
  ...
  try{
    ...
    scene.setParams(state.params);$('#scene-loading').hidden=true;
...
function apply(next){
  ...
  if(!parent){
    ...
    $('#next-title').textContent=nextReward?nextReward.title:'所有成长门槛都达成啦';
    $('#next-description').textContent=nextReward?(nextReward.cost===0?'达到门槛，自动收入你的收藏。':`达到门槛后，可用 ${nextReward.cost} 积分兑换。`):'慢慢收集你喜欢的装扮，和小猫一起享受成长。';
    $('#growth-progress').max=nextReward?.unlockAt||1;$('#growth-progress').value=nextReward?state.lifetime:1;
    $('#next-points').textContent=nextReward?`${state.lifetime} / ${nextReward.unlockAt} 成长积分`:'每一次努力，仍然值得被鼓励。';
...
document.addEventListener('click',event=>{
  ...
  if(action==='category'){category=b.dataset.category;renderRewards();return;}
