...
try {
  ...
  // Confirmed table discard must not leave stale values or a dirty marker behind.
  const priceInput=parent.locator('#catalog-body input[data-reward-id="coat-grey"][data-field="cost"]');
  const savedPrice=await priceInput.inputValue();await priceInput.fill('123');
  parent.once('dialog',d=>d.dismiss());await parent.locator('[data-action=new-preset]').click();
  assert.equal(await priceInput.inputValue(),'123');assert.equal(await parent.locator('#preset-dialog').isVisible(),false);
  parent.once('dialog',d=>d.accept());await parent.locator('[data-action=new-preset]').click();
  await parent.locator('#preset-dialog').waitFor({state:'visible'});await parent.locator('#preset-close').click();
  await parent.locator('#preset-dialog').waitFor({state:'hidden'});
  assert.equal(await priceInput.inputValue(),savedPrice);assert.equal(await parent.locator('#catalog-dirty').isVisible(),false);
  ...
  await parent.screenshot({path:fileURLToPath(new URL('failure-parent.png',out)),fullPage:true}).catch(()=>{});
  await child.screenshot({path:fileURLToPath(new URL('failure-child.png',out)),fullPage:true}).catch(()=>{});
