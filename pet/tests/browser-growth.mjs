...
try{
  ...
  await parent.getByRole('button',{name:'查看依据并确认',exact:true}).click();await parent.locator('#growth-confirmed').check();await parent.locator('#award-submit').click();await parent.waitForFunction(()=>document.querySelector('#balance').textContent==='5');mark('孩子提交不直接入账，家长按快照确认后增加3分');
  ...
  await parent.locator('#history-category').selectOption('responsibility');await parent.locator('#history-entries').getByRole('button',{name:'更正误录',exact:true}).click();await parent.locator('#growth-dialog [name=reason]').fill('测试误录更正，关联原记录');await parent.getByRole('button',{name:'确认保存',exact:true}).click();await parent.locator('#growth-dialog').waitFor({state:'hidden'});assert.equal(app.store.snapshot().balance,5);assert.equal(app.store.snapshot().lifetime,8);mark('误录更正关联原流水，不透支、不回收收藏');
  ...
  await parent.locator('[data-parent-tab=award]').click();await parent.locator('[data-growth-category=creativity]').click();await parent.locator('#growth-task').selectOption('');await parent.locator('#growth-title').fill('画出自己的想法');await parent.locator('#reason').fill('自愿画了自己喜欢的颜色');await parent.locator('#delta').selectOption('0');await parent.locator('#growth-confirmed').check();await parent.locator('#award-submit').click();await parent.waitForFunction(()=>document.querySelector('#reason').value==='');assert.equal(app.store.snapshot().balance,5);assert.equal(app.store.history({kind:'observation'}).total,1);mark('仅记录保存成长回忆，不增加喵币或经验');
