...
try{
  ...
  await parent.locator('[data-award-task=school_basic-health-1]').click();
  assert.ok((await parent.locator('#growth-basis').innerText()).length>0);assert.equal(await parent.locator('#growth-basis').isVisible(),true);
  ...
  await parent.getByRole('button',{name:'管理目标与项目',exact:true}).click();assert.equal(await parent.locator('.growth-library .growth-task-card').count(),18);
  ...
  await parent.getByRole('button',{name:'年龄 / 模式',exact:true}).click();await parent.locator('#growth-dialog [name=age]').selectOption('9');assert.equal(await parent.locator('#growth-dialog [name=mode]').inputValue(),'school_basic');await parent.locator('#growth-dialog [name=mode]').selectOption('school_advanced');await parent.getByRole('button',{name:'确认并保存',exact:true}).click();await parent.locator('#growth-dialog').waitFor({state:'hidden'});assert.equal(app.store.growth.profile().mode,'school_advanced');assert.equal(app.store.growth.read().tasks.length,1);assert.equal(app.store.snapshot().balance,5);mark('年龄不自动切模式，手动进阶保留目标与余额');
  await parent.getByRole('button',{name:'管理目标与项目',exact:true}).click();await parent.getByRole('button',{name:'新增自定义项目',exact:true}).click();
  ...
  await parent.locator('[data-parent-tab=growth]').click();await parent.locator('#history-category').selectOption('unclassified');await parent.locator('#history-entries').getByRole('button',{name:'给旧记录归类',exact:true}).click();await parent.locator('#growth-dialog [name=category]').selectOption('responsibility');await parent.locator('#growth-dialog [name=reason]').fill('家长确认原记录为整理桌面');await parent.getByRole('button',{name:'确认保存',exact:true}).click();await parent.locator('#growth-dialog').waitFor({state:'hidden'});assert.equal(app.store.snapshot().balance,8);assert.equal(app.store.growth.read(true).unclassified,0);mark('旧记录手动归类不重复发放积分');
  ...
  await parent.locator('[data-parent-tab=growth]').click();assert.equal(await parent.locator('.growth-summary-card').count(),6);assert.match(await parent.locator('.growth-summary').innerText(),/每一份进步/);await screenshot(parent,'growth-family-distribution');
  await parent.locator('[data-parent-tab=award]').click();await parent.locator('[data-growth-category=creativity]').click();await parent.locator('[data-award-task=other]').click();await parent.locator('#reason').fill('画出自己的想法');await parent.locator('[name=scoreChoice][value="0"]').check();await parent.locator('#award-submit').click();await parent.waitForFunction(()=>document.querySelector('#reason').value==='');assert.equal(app.store.snapshot().balance,5);assert.equal(app.store.history({kind:'observation'}).total,1);mark('仅记录保存成长回忆，不增加喵币或经验');
