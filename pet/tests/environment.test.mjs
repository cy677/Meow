...
test('theme metadata can be edited when member keys are reordered, but members stay immutable',t=>{
 const s=fresh(t);award(s);buy(s,'scene-theme-cozy');s.equip('scene-theme-cozy');
 const before=s.snapshot(),history=s.exportData().ledger;
 const reward=s.catalog().rewards.find(r=>r.id==='scene-theme-cozy');
 reward.params.members=Object.fromEntries(Object.entries(reward.params.members).reverse());
 reward.title='温馨小屋新名字';reward.cost+=5;
 const after=s.savePreset(reward,s.catalogRevision());
 assert.equal(after.balance,before.balance);assert.deepEqual(after.owned,before.owned);
 assert.deepEqual(after.equipped,before.equipped);assert.deepEqual(after.sceneParams,before.sceneParams);
 assert.equal(s.catalog().rewards.find(r=>r.id===reward.id).title,reward.title);
 for(const row of history)assert.deepEqual(s.exportData().ledger.find(item=>item.id===row.id),row);
 for(const members of [
   {...reward.params.members,floor:'scene-floor-blue'},
   {...reward.params.members,camera:'scene-camera-room'},
   Object.fromEntries(Object.entries(reward.params.members).filter(([slot])=>slot!=='floor')),
 ])assert.throws(()=>s.savePreset({...reward,params:{members}},s.catalogRevision()),e=>e.status===409);
});
