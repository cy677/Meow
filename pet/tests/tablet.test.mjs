...
test('原生 HTTPS 正确验证证书并设置 Secure Cookie；缺失证书启动失败',async t=>{
  ...
  const key=join(dir,'key.pem'),cert=join(dir,'cert.pem'),config=join(dir,'openssl.cnf');
  // Test certificates must not depend on a machine-wide OpenSSL config path.
  writeFileSync(config,'[req]\ndistinguished_name=dn\n[dn]\n');
  execFileSync('openssl',['req','-config',config,'-x509','-newkey','rsa:2048','-nodes','-keyout',key,'-out',cert,'-days','1','-subj','/CN=localhost','-addext','subjectAltName=DNS:localhost,IP:127.0.0.1'],{stdio:'pipe'});
