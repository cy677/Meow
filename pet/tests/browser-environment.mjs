...
try{
 ...
 // Resetting a saved theme preserves its locked members and remains saveable.
 await parent.locator('#catalog-filter').selectOption('theme');
 await parent.locator(`[data-action=edit-preset][data-id="${bundle.id}"]`).tap();
 await parent.locator('#preset-reset').tap();
 for(const [slot,id]of Object.entries(bundle.params.members)){
   assert.equal(await parent.locator(`#preset-param-${slot}`).inputValue(),id);
   assert.equal(await parent.locator(`#preset-param-${slot}`).isDisabled(),true);
 }
 await parent.locator('#preset-form [name=title]').fill('我的场景套装（更新）');
 await parent.locator('#preset-save').tap();await parent.locator('#preset-dialog').waitFor({state:'hidden'});
 const updatedBundle=app.store.catalog().rewards.find(r=>r.id===bundle.id);
 assert.equal(updatedBundle.title,'我的场景套装（更新）');assert.deepEqual(updatedBundle.params.members,bundle.params.members);
