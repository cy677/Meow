export function createApiClient({getCsrf=()=>'',fetchImpl=globalThis.fetch,timeoutMs=12000}={}) {
  return async function request(path,method='GET',data,extraHeaders={}) {
    ...
    try {
      const response=await fetchImpl(path,{method,credentials:'same-origin',cache:'no-store',signal:controller.signal,
