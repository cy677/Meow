import { createModelRewardsLayer } from '../models/modelLayer.js';
// Shared renderer extension, owned by the pet runtime adapter.
...
import { createClipPlayer as petCreateClipPlayer } from '../../src/catMotion/clipPlayer.js';
import { createMotionController as petCreateMotionController } from '../../src/catMotion/motionController.js';
import { createMotionEvents as petCreateMotionEvents } from '../../src/catMotion/motionEvents.js';
import { planMotion as petPlanMotion } from '../../src/catMotion/motionScript.js';
...
function petEnsureMotion() {
  if(!host.motionRig||!host.cat)throw new Error('小猫骨骼尚未准备好');
  if(petMotionRig!==host.motionRig) {
...
function petValidateMotionAccess(action,motion) {
  if(petAccess&&!petAccess.actions.some(a=>a.action===action&&JSON.stringify(a.motion||{})===JSON.stringify(motion||{})))throw new Error('此动作尚未获得使用权限');
}
let petChildMotion=false,petLastElapsed=0;
let petWanderPlan=null;
...
function petAdvanceWander(plan,elapsed){
  if(petWanderPlan!==plan){petWanderPlan=plan;petLastElapsed=elapsed;}
...
const petStudio = {
  ...
  sampleMotion:petSampleMotion,photo:host.photo,animationState:host.animationState,applyModels:petApplyModels,disposeModels:petDisposeModels,modelDiagnostics:()=>petModelLayer?.diagnostics()||{items:[],errors:[],loading:0},
  resetView(view) {
    ...
    controls.update();controls.enableDamping=damping;petModelLayer?.fitView();
  },
  setAccess(access) {
    petChildMotion=true;petAccess=structuredClone(access);
    host.photo.setPoseControl({
      options:()=>petAccess.poseOptions||[],current:()=>params.pose,
      select:option=>{petStudio.stop(true);host.staticPoseBeforeMotion=option.params.pose;host.setParams(option.params);},
    });
  ...
  motion:{
    ...
    enqueue(action,motion={},options={}){return this.play(action,motion,{...options,enqueue:true});},
    stop(options={}){petStudio.stop(options.immediate===true);},
    pause(){petMotion?.pause();},resume(){petMotion?.resume();},
    on:petMotionEvents.on,
    setTarget(slot,point){petEnsureMotion().setTarget(slot,point);},
    getTargets(){return petMotion?.getTargets()||{};},
  ...
  lock(){if(locked)return;locked=true;window.removeEventListener('pagehide',petDisposeModels);toyWorld.step=petOriginalToyStep;petMotion?.dispose();petMotionEvents.dispose();petDisposeModels();bgm.pause();weatherAudio.setRain(false);motionMachine.clearKeys();renderer.setAnimationLoop(null);},
};

// Share the existing physical world and render clock; never create another animation loop.
let petModelLayer=null;
const petOriginalToyStep=toyWorld.step;
toyWorld.step=dt=>{petOriginalToyStep(dt);petModelLayer?.update(dt);};
function petApplyModels(ids){
  if(!petModelLayer&&!ids.length)return;
  petModelLayer??=createModelRewardsLayer({scene,camera,canvas:renderer.domElement,controls,toyWorld,getCat:()=>host.cat});
  void petModelLayer.select(ids);
}
function petDisposeModels(){petModelLayer?.dispose();petModelLayer=null;}
window.addEventListener('pagehide',petDisposeModels);

// Stable developer-facing animation facade; household permissions still apply.
window.meowMotion=petStudio.motion;
