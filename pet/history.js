  ...
  function render() {
    ...
    for (const record of entries) {
      ...
      if(growth){const detail=node('details','history-detail');detail.append(node('summary','','查看当时的加分依据'));
        for(const line of [growth.task?.condition&&`当时完成条件：${growth.task.condition}`,growth.assistance&&`帮助：${growth.assistance}`,growth.scoreReason&&`分值调整：${growth.scoreReason}`,growth.classificationReason&&`旧记录归类依据：${growth.classificationReason}`,growth.correctionOf&&`更正原记录：${growth.originalReason}`,record.occurredAt&&`行动时间：${new Date(record.occurredAt).toLocaleString('zh-CN')}；录入时间见上方`].filter(Boolean))detail.append(node('p','note',line));content.append(detail);}
