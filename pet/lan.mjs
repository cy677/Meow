...
export function normalizePublicIp(value = '') {
  if (typeof value !== 'string') throw new Error('MEOW_PUBLIC_IP 必须是服务器 IP，不含协议、端口或路径');
  const ip = value.trim();
  if (!ip) return '';
  const version = isIP(ip);
  if (!version) throw new Error('MEOW_PUBLIC_IP 必须是服务器 IP，不含协议、端口或路径');
...
export function launchOptions(args = [], env = process.env) {
  ...
  return { host: env.MEOW_HOST || (lan ? '0.0.0.0' : '127.0.0.1'), port, dev, useTls,
    publicIp: normalizePublicIp(env.MEOW_PUBLIC_IP), openBrowser: args.includes('--open'),
...
export function accessUrls({ host, port, useTls, addresses = lanAddresses(), publicIp = '' }) {
  const protocol = useTls ? 'https' : 'http';
  const hosts = host === '0.0.0.0' || host === '::' ? ['localhost', ...addresses, ...(publicIp ? [publicIp] : [])] : [host];
