...
`occurredAt` 必须为带时区的有效ISO时间，不接受未来行动（仅允许5分钟时钟误差）。`delta` 可选0/1/2/3/5，0表示仅记录。已有项目的 `reason` 可省略或为空，此时流水使用项目名称；按步骤计分时同时附上步骤名称。普通项目可选择本次分值，`scoreReason` 为兼容保留的选填字段。按步骤计分需 `stepId`，不能领取总分或临时改步骤积分。
...
“其他”（不传 `taskId`）必须提供六类之一的 `category`、非空 `title` 和具体 `reason`，空白理由会被拒绝。家长页面统一发送 `title: "其他"`，流水保留填写的具体理由。日常加分按活力、心情、友爱、能干、好奇、创意切换项目和分值；成长记录页支持按同样六类筛选历史。
