...
export function createPresetEditor({ api, onSaved }) {
  ...
  for (const [id, label] of Object.entries(CATEGORY_LABELS).filter(([id])=>!['creation','capability','model'].includes(id))) { const option = node('option', '', label); option.value = id; form.elements.category.append(option); }
