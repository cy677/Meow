...
**中文** · [English](README.en.md) · [日本語版说明](README.ja.md)

一个把现实中的小小努力，变成成长回忆、小猫装扮和房间礼物的本地家庭应用。

**3—10岁 · 三种成长模式 · 六类成长记录 · 家长确认加分 · 本地保存**
...
**家庭版有本地账户和Node.js服务，数据保存在服务电脑的SQLite数据库中。** iPad浏览器访问同一个本地服务；积分、加分理由、奖励预设和兑换记录不以浏览器缓存为唯一账本，不上传到GitHub或云端。原版静态生成器的“无后端、无需登录”说明不适用于家庭积分服务。
...
- **互动小猫与房间**：保留原版软体触摸、14个原版动作、键盘与触屏移动、物理玩具、光照、天气、拍照和导出。
...
- **本地奖励与数据管理**：喵币与累计经验分开；家长保存固定外观和场景作品；支持旧数据迁移、记录查询、JSON导出及数据库备份。
...
已兑换物品重复使用不再次收费。现有目录的逐步揭晓和场景装备规则继续保留；原版14个动作、抚摸、拍照及已有免费导出功能不因缺积分而失去。参数编辑和随机生成仅供家长使用。
...
家长页可导出完整JSON记录，不包含密码或会话。**JSON用于查阅，不提供自动导入恢复。** 恢复备份应先停止服务，再恢复完整SQLite数据目录。关闭网页或清理浏览器缓存不会删除服务电脑上的数据库。
...
本版尚未接入语音大模型、相机合影或心理自动判断。家长功能与儿童权限在服务端分离；分类与分值用于家庭记录和鼓励，不用于心理筛查、诊断或儿童之间的比较。
