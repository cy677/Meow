...
完整多用户、新模型、完整成长图表、云同步与大规模性能优化均未实现。设备摄像头在本分支后续的第一版合影中接入：仅用户主动点击后申请权限，照片只在浏览器内存处理，经系统菜单保存，不写数据库或网页相册。见 [合影说明](photo-v1.md)。模型接口只是扩展点，新模型仍需编写资源和动作适配。
...
阶段1—5基线的历史 Node 22.16.0 检查记录（不是后续合影提交的测试结果）：125项中122通过、0失败、3项既有 Windows 专用测试跳过；积分版与原版构建通过。真实 iPad 和 Windows 便携版未实测。旧导航专用浏览器脚本留作历史参考，不计入本轮有效覆盖；当前入口为 `pet/tests/browser-maintenance.mjs`。
