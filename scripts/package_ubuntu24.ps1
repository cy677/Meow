...
$name = 'Meow-V6-Ubuntu24-x64-' + (Get-Date -Format 'yyyyMMdd-HHmmss')
...
try {
    ...
    foreach ($file in @('package.json','pet-settings.json','LICENSE','COMMERCIAL-LICENSE.md','Meow')) { Copy-Item -LiteralPath (Join-Path $root $file) -Destination $out }
    ...
    @'
Meow V6 — Ubuntu 24.04 x86_64 便携版

1. 解压：tar -xzf Meow-V6-Ubuntu24-x64-*.tar.gz
2. 进入解压后的目录：cd Meow-V6-Ubuntu24-x64-*
3. 启动：./Meow
4. 停止：在启动终端按 Ctrl+C
