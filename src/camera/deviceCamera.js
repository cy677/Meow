...
export function createDeviceCameraProvider({
  ...
  async function start({facingMode = 'user', deviceId} = {}) {
    ...
    try {
      ...
      state = 'ready';
      return {video, facingMode: facing, deviceId: settings.deviceId, width: video.videoWidth, height: video.videoHeight};
    } catch (error) {
