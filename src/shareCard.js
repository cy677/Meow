...
export function createShareCardCapture({viewport, renderer, scene, camera, controls, sceneCanvas, getSubject, getSeed, getPalette, getLocale, downloadBlob}) {
  ...
  let device, systemPhotosOnly = false;
  ...
  async function capture({download = true} = {}) {
    ...
    try {
      const viewRect = windowEl.getBoundingClientRect(), canvasRect = sceneCanvas.getBoundingClientRect();
      ...
      const output = document.createElement('canvas'); output.width = 1200; output.height = 1600;
      ...
      if (together || (systemPhotosOnly && download)) device.showPreview(value); // Family photos never auto-download.
      else if (download) await downloadBlob(blob, value.filename); // Standalone generator compatibility.
      if (request !== generation || disposed) return null;
      statusEl.textContent = together ? '照片已生成，相机已关闭。请通过系统菜单存储到照片。' : localeCopy(getLocale()).saved;
  ...
  return {open, close, capture, setOutputMode(mode) { systemPhotosOnly = mode === 'photos'; }, get active() { return active; }, dispose() {
